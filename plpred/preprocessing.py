...
def compute_aa_composition(protein_sequence:str) ->dict:
    """    
    Computer the aminoacid composition of a given protein sequence.
...
def generate_aa_composition_df(file_path:str, membrane_label:int) -> pd.DataFrame:
    """
    Generates a Pandas DataFrame containing the amino acid composition for each protein in an FASTA file.
    ...
    membrane_label: int
      label indicating if the protein is located in membrane (1) or cytoplasm (0)
